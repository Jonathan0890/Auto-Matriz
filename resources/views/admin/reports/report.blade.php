...
@section('content')
    <h1 class="text-3xl font-medium">REPORTE</h1>
        ...
        <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow-lg">
            <thead class="bg-[#b1b1b1] text-white">
                <tr>
                    ...
                    <th class="py-3 px-6 text-left text-xs font-medium uppercase tracking-wider">Tipo</th>
                    <th class="py-3 px-6 text-left text-xs font-medium uppercase tracking-wider">Descripcion</th>
            ...
            <tbody class="text-gray-700">
                @foreach ($reportes as $report)
                    <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-200">
                        <td class="py-4 px-6 whitespace-nowrap">{{ $report->id }}</td>
                        <td class="py-4 px-6 whitespace-nowrap">{{ $report->nombre }}</td>
                        <td class="py-4 px-6 whitespace-nowrap">{{ $report->email }}</td>
                        <td class="py-4 px-6 whitespace-nowrap">{{ $report->tipo }}</td>
                        <td class="py-4 px-6 whitespace-nowrap">{{ $report->tipo}}</td>
                        <td class="py-4 px-6 whitespace-nowrap"><a href="{{ route('reporte.show', $report->id) }}"
                                class="text-blue-500 hover:text-blue-700">Mostrar</a></td>
                        <td class="py-4 px-6 whitespace-nowrap"><a href="{{ route('reporte.edit', $report->id) }}"
                                class="text-yellow-500 hover:text-yellow-700">Editar</a></td>
                        <td class="py-4 px-6 whitespace-nowrap">
                            <form action="{{ route('reporte.destroy', $report->id) }}" method="POST"
